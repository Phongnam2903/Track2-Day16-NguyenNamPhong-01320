...
| Metric                           | Kết quả                                 |
| -------------------------------- | ----------------------------------------- |
| Thời gian load data             | 2.71 s (227,845 train / 56,962 test rows) |
| Thời gian training              | 21.23 s                                   |
| Best iteration                   | 0                                         |
| AUC-ROC                          | 0.8928                                    |
| Accuracy                         | 0.9989                                    |
| F1-Score                         | 0.6935                                    |
| Precision                        | 0.6832                                    |
| Recall                           | 0.7041                                    |
| Inference latency (1 row)        | 1.698 ms                                  |
| Inference throughput (1000 rows) | 39,736 rows/s (0.0252 s cho 1000 dòng)   |
...
**Kết quả thực tế đo được (sau khi chạy benchmark):**

> Lưu ý: Compute Node thực tế đang chạy là `t3.micro` (1 vCPU khả dụng qua `nproc`=2, ~914 MB RAM), không phải `t3.medium` (4 GB RAM) như mô tả — do `variables.tf` đặt `default = "t3.micro"` cho biến `cpu_instance_type`, khác với con số ghi trong tài liệu. Nếu muốn đúng theo bài (2 vCPU/4GB), cần sửa default này thành `t3.medium` rồi `terraform apply` lại. Số liệu dưới đây là đo trên `t3.micro` thực tế đang chạy.

CPU gần như idle (100.0 id) tại thời điểm đo (`top` chụp sau khi benchmark đã chạy xong), RAM dùng ~210 MB / 914 MB, đã nhận ~284 MB dữ liệu qua mạng (chủ yếu là tải dataset Kaggle ~150MB + cài package).

*(Ảnh chụp màn hình thật của terminal/EC2 Monitoring vẫn cần tự chụp để nộp bài — đây là dữ liệu text lấy tự động qua SSH, không thay thế được screenshot.)*
...
**Bản nháp báo cáo ngắn (mục 6):**

> Trên một Compute Node CPU-only (`t3.micro`, ~1 GB RAM, không GPU, không cần xin quota), việc load 284,807 dòng dữ liệu Credit Card Fraud mất 2.71s và huấn luyện LGBMClassifier (500 cây) mất 21.23s — cho thấy LightGBM rất phù hợp để chạy baseline nhanh trên phần cứng nhỏ, không cần hạ tầng GPU đắt đỏ. Model đạt AUC-ROC 0.8928 và Accuracy 99.89%, ở mức chấp nhận được cho một baseline chưa tune trên bài toán cực mất cân bằng (492/284,807 giao dịch là gian lận); Precision/Recall quanh 0.68-0.70 phản ánh đúng đặc thù đánh đổi giữa bắt được gian lận và số cảnh báo sai. Về tốc độ inference, latency chỉ 1.7ms cho 1 giao dịch và throughput đạt ~39,700 giao dịch/giây khi dự đoán theo lô — đủ nhanh để phục vụ real-time scoring trong hệ thống thanh toán mà không cần phần cứng chuyên dụng. Tổng chi phí hạ tầng cho toàn bộ quá trình test (EC2 CPU + Bastion + NAT Gateway + ALB) chỉ khoảng $0.10/giờ, minh chứng cho việc luồng CPU + LightGBM là lựa chọn hiệu quả chi phí cho các bài toán ML dạng bảng (tabular) không cần đến LLM hay GPU.

*(Bạn có thể chỉnh sửa đoạn trên theo trải nghiệm thực tế của mình trước khi nộp — đây chỉ là bản nháp dựa trên số liệu benchmark đã đo được.)*
